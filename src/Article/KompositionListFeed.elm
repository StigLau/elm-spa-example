module Article.KompositionListFeed exposing (Model, Msg, init, update, viewPagination, viewTabs, kompdecoder)
...
-}
type alias Internals = List String
    {--
    { session : Session
    , errors : List String
...
init : Session -> (List String) -> Model
...
{--
decoder : Maybe Cred -> Int -> Decoder Int
decoder maybeCred resultsPerPage =
    Decode.int |> required "articles" --PaginatedList.fromList
...
kompdecoder : Maybe Cred -> Int -> Decoder (List String)
kompdecoder maybeCred resultsPerPage =
    Decode.succeed []
